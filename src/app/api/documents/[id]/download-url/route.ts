...
export async function GET(_: Request, context: { params: RouteParams }) {
  ...
  if (!id) return NextResponse.json({ error: 'ID de documento invalido.' }, { status: 400 })
  ...
  if (!doc.approved) {
    return NextResponse.json({ error: 'Documento no disponible para descarga.' }, { status: 403 })
  }
  ...
  if (isHttpUrl(rawPath)) {
    ...
    if (!parsed) return NextResponse.json({ url: rawPath })
    ...
    for (const key of candidates) {
      ...
      if (!signedError && signed?.signedUrl) return NextResponse.json({ url: signed.signedUrl })
    }
  ...
  return NextResponse.json({ error: 'No se pudo generar URL de descarga.' }, { status: 500 })
}
