...
export async function POST(_: Request, context: { params: RouteParams }) {
  ...
  try {
    ...
    let accessMode: 'owner' | 'premium' | 'free' | 'contributor' | 'points' | 'repeat' = 'owner'
    ...
    if (!isOwner) {
      ...
      try {
        const { data: priorDownload } = await supabaseAdmin
          .from('audit_logs')
          .select('id')
          .eq('user_id', user.id)
          .eq('action', 'download')
          .contains('details', { document_id: id })
          .maybeSingle()
        hasDownloadedBefore = Boolean(priorDownload?.id)
      } catch {
        // Si audit_logs no existe o no es consultable, seguimos con flujo normal.
      }
    ...
    try {
      ...
      // No bloqueamos la descarga si falla el log.
    }
