...
export default function DashboardPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const { data: rankData, error: rankError } = await supabase.rpc('get_user_rank', { uid: user.id })
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto mt-2 mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard label="Puntos Acumulados" value={stats.puntos} color="text-purple-600 dark:text-purple-400" />
          <StatCard label="Ranking Global" value={ranking !== null ? `#${ranking}` : 'N/A'} color="text-yellow-500 dark:text-yellow-400" />
          <StatCard label="Medalla" value={`${medalla.medalla} ${medalla.nivel}`} color={medalla.color} />
        </div>
            ...
            {lineData.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={lineData}>
                  ...
                  <Tooltip formatter={(value: number) => `${value} doc(s)`} />
                  <Legend />
                  <Line type="monotone" dataKey="count" name="Documentos" stroke="#10b981" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : <NoDataMessage />}
          </ChartCard>
...
function StatCard({ label, value, color }: { label: string; value: number | string; color?: string }) {
  return (
    <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-5 transform transition hover:scale-105">
      ...
      <p className={`mt-1 text-3xl font-semibold ${color || 'text-gray-900 dark:text-white'}`}>{value}</p>
    </div>
  )
}
