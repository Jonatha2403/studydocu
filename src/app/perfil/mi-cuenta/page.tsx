...
import { Loader2, User2, Edit3, Save, ListChecks, ShieldCheck, Award, Mail, CreditCard } from 'lucide-react'
...
function HistorialAcciones({ userId }: { userId: string }) {
  ...
  if (loading) return <p className="text-sm text-gray-500 dark:text-gray-400">Cargando historial...</p>
  ...
  if (!user || !profile) {
    return (
      <div className="text-center mt-10">
        ...
        <Link href="/auth?modo=login" className="mt-3 inline-block bg-blue-600 text-white px-4 py-2 rounded-md">
          Iniciar Sesión
        </Link>
      </div>
    )
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className={`text-sm flex items-center gap-2 ${profile.subscription_status === 'Activa' ? 'text-green-600' : 'text-red-600'}`}>
        ...
        {isEditing && (
          <form onSubmit={handleSubmit} className="space-y-4 bg-slate-50 dark:bg-slate-800 p-4 rounded-md">
            {['username', 'nombre', 'carrera', 'universidad'].map(field => (
              <div key={field}>
