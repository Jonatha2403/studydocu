...
export default function ActivityHistory({ userId }: Props) {
  ...
  const getIcon = (action: string) => {
    switch (action) {
      case 'document_uploaded': return <UploadCloud className="w-5 h-5 text-blue-600" />
      case 'comment_posted': return <MessageCircle className="w-5 h-5 text-green-600" />
      case 'document_saved': return <Star className="w-5 h-5 text-yellow-500" />
      case 'report_sent': return <AlertTriangle className="w-5 h-5 text-red-500" />
      case 'achievement_unlocked': return <Trophy className="w-5 h-5 text-purple-600" />
  ...
  return (
    <div className="mt-6 space-y-4">
      {activities.map((item) => (
        <div key={item.id} className="flex items-start gap-3 p-3 border rounded-md bg-white shadow-sm">
