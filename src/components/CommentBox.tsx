...
export default function CommentBox({ documentId, user }: CommentBoxProps) {
  ...
  }, [newComment, user, documentId, fetchComments])
    ...
    <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 selection:bg-yellow-400 selection:text-black">
      ...
      {user ? (
        <form onSubmit={handleSubmit} className="mb-6 space-y-3">
          ...
          >
            {isSubmitting ? <><Loader2 className="animate-spin w-4 h-4" /> Enviando...</> : <><Send size={16} /> Enviar</>}
          </button>
        </form>
      ) : (
        <p className="mb-6 text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700/50 p-3 rounded-md text-center">
          Debes <a href="/auth" className="text-blue-600 dark:text-yellow-400 font-semibold underline">iniciar sesión</a> para comentar.
        </p>
      )}
      ...
      {loadingComments ? (
        ...
        <p className="text-center text-gray-500 dark:text-gray-400 py-8">No hay comentarios. ¡Sé el primero!</p>
      ) : (
        <ul className="space-y-4">
          {comments.map((comment) => {
            const profile = comment.profiles?.[0]
            return (
              <li key={comment.id} className="p-4 rounded-lg bg-gray-50 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
