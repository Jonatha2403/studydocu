...
export default function MisionesDiarias({ userId }: { userId: string }) {
  ...
  const cargarMisiones = async () => {
    const { data } = await supabase.rpc('get_user_daily_missions', { uid: userId })
  ...
  const completarMision = async (m: Mision) => {
    ...
    // Mostrar animación / notificación
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-md mt-6">
      ...
      {misiones.length === 0 && <p className="text-sm text-gray-500">No hay misiones cargadas.</p>}
        ...
        {misiones.map((m) => (
          ...
          >
            ...
            <div className="text-xs mt-1">
              ...
              {m.completed && <span className="ml-2 text-green-600 font-semibold">✔ Completada</span>}
            </div>
