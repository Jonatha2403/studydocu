...
export default function RankingExploradores() {
  ...
  useEffect(() => {
    const obtenerRanking = async () => {
      ...
      // NO forzar tipo aquí, revisamos dentro del loop
      const acumulado: Record<string, number> = {}

      data.forEach((doc: any) => {
        const username = doc.profiles?.username
        ...
        {ranking.length > 0 ? (
          ranking.map((item, index) => (
            <li key={index} className="flex justify-between items-center">
              <Link href={`/perfil/usuario/${item.username}`} className="hover:underline text-primary">
                @{item.username}
              </Link>
