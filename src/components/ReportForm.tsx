...
import { AlertTriangle, Send, CheckCircle, Loader2 } from 'lucide-react' // Iconos

interface ReportFormProps {
  documentId: string;
  userId: string | null; // Permitir null si el usuario puede no estar logueado
}
...
export default function ReportForm({ documentId, userId }: ReportFormProps) {
  ...
  const handleSubmit = useCallback(async (event?: FormEvent<HTMLFormElement>) => {
    if (event) event.preventDefault(); // Prevenir recarga de página al usar <form onSubmit>
    ...
    try {
      const { error } = await supabase.from('reports').insert({
        ...
        reason: trimmedReason, // Enviar razón sin espacios extra
      });

      if (error) throw error; // Lanzar para que lo capture el catch

      toast.success('✅ Reporte enviado correctamente.');
      setReason('');
      setSubmitted(true);
    } catch (err: any) {
      console.error("Error submitting report:", err); // Log completo del error para desarrollo
  ...
  }, [userId, documentId, reason]); // Dependencias de useCallback

  if (!userId && !submitted) { // Si no hay usuario y aún no se ha enviado nada (ej. para mostrar un login)
    return (
      <div className="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 rounded-md text-yellow-700 dark:text-yellow-300 text-sm text-center">
        ...
        Debes <a href="/auth?modo=login" className="font-semibold underline hover:text-yellow-800 dark:hover:text-yellow-200">iniciar sesión</a> para poder reportar un documento.
      </div>
    );
  }
  ...
  return (
    <form 
      ...
      className="mt-8 p-4 sm:p-6 bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 rounded-lg shadow-md selection:bg-red-200 selection:text-red-800"
    >
