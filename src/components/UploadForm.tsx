...
interface UploadFormProps {
  user: User | null;
  onUploadComplete: (uploadedDocumentId?: string, uploadedDocumentContent?: string) => Promise<void> | void;
  disabled?: boolean;
}
...
export default function UploadForm({ user, onUploadComplete, disabled: parentDisabled }: UploadFormProps) {
  ...
  const [uploadedDocs, setUploadedDocs] = useState<DocumentEntry[]>([])
  ...
  const registrarDocumento = async (fileToRegister: File, hash: string, docCategory: string): Promise<{id: string, content: string}> => {
    if (!user) throw new Error("Usuario no autenticado.");

    const filePath = `${user.id}/${Date.now()}_${fileToRegister.name}`

    const { error: uploadError } = await supabase.storage.from('documents').upload(filePath, fileToRegister, {
      upsert: false,
    });
    ...
    if (insertError) throw new Error('Error al guardar metadatos del documento: ' + insertError.message)
    if (!newDocument || !newDocument.id) throw new Error('No se pudo obtener el ID del nuevo documento.');

    await supabase.from('audit_logs').insert({
      user_id: user.id,
      action: 'document_uploaded',
      details: { document_id: newDocument.id, file_name: fileToRegister.name, category: docCategory },
    });
  ...
  const handleUpload = async () => {
    ...
    if (recentUpload?.length && new Date(recentUpload[0].created_at).getTime() > Date.now() - 2 * 60 * 1000) {
    ...
    try {
      ...
      setFileHash(null)
      setProgress(0)
      setUploadedDocs((prev: DocumentEntry[]) => [ {
        id: newDocDetails.id,
        file_name: file.name,
