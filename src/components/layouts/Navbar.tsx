...
import {
  Menu, X, Home, UploadCloud, LogIn, UserPlus,
  CreditCard, ShieldCheck, Loader2, Search, Briefcase
} from 'lucide-react'
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { href: '/servicios', label: 'Servicios', icon: Briefcase }, // ✅ Nuevo enlace
  ...
  return (
    <nav className={`sticky top-0 z-50 transition-all duration-300 ${menuOpen ? 'bg-white/90 shadow-md backdrop-blur-lg dark:bg-gray-900/80' : 'bg-white/80 backdrop-blur-lg shadow-sm dark:bg-gray-900/70'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">

        <Link href="/" className="text-xl font-bold text-primary hover:opacity-90 transition-all flex items-center gap-2">
          📘 StudyDocu
        </Link>
        ...
        {menuOpen && (
          ...
          >
            ...
            <div className="pt-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
              {user ? (
                <UserMenu userProfile={perfil} loading={loading} isMobile closeMobileMenu={closeMenu} />
              ) : (
                <>
